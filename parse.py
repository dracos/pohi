...
def parse_transcript(url, text):
    ...
    for page, num, line in data:
        ...
        # Date at start
        m = re.match(' *(Mon|Tues|Wednes|Thurs|Fri)day,? \d+(nd)? (August|September|October|November|December|January|February|March|April|May|June|July) 20[12][123]$', line)
        if m:
        ...
        # Headings
        m = re.match('Focus Group Session [34]$|Housekeeping$|((Opening|Closing) s|S)tatement by [A-Z ]*(?:, QC)?(?: \(continued\))?$|[A-Z ]*$|Announcements by [A-Z ]*$|(Further s|S)ubmissions? by [A-Z ]*(?:, QC)?$|Reply by [A-Z ]*$|Decision$', line.strip())
        if m:
            yield speech
