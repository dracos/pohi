...
def load_data():
    ...
    for name in sorted(glob.glob('evidence/*.rst') + glob.glob('evidence-2/*.rst')):
        if m := re.match('evidence(?:-2)?/([A-Za-z]{3,4}[0-9_]+(r|R|ds)?)', name):
...
def parse_transcript(url, text):
    ...
    for page, num, line in data:
        ...
        m = re.match('(?:(?:Opening|Closing) (?:remarks|submissions|statement)|Statement) (?:by|from) ([A-Z ]*)(?:, QC)?(?: \(continued\))?$|Announcements by ([A-Z ]*)$|(?:Further s|S)ubmissions? by ([A-Z ]*)(?:, QC)?$|Reply by ([A-Z ]*)$|Witness statement of ([A-Zc ]*) adduced$', line.strip())
        if m:
            yield speech
